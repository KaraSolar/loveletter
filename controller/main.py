...
from .workers import WorkerDatabase, WorkerModbus, WorkerCanBusGps
...
class Controller:
    def __init__(self, view, db_name, telemetry_config: dict, passenger_number_config: dict, trip_purposes_config: list):
        ...
        if self.telemetry_config["target"] == "cerbo_gx":
            ...
            self.worker_get_telemetry = WorkerCanBusGps(queue_worker_database=self.queue_worker_database,
                                                        ...
                                                        trip_mode_flag_signal=self.trip_mode_flag_signal,
                                                        queue_view=self.queue_view,
                                                        channel_config=self.telemetry_config["config"])
        self.worker_get_telemetry.start()
    ...
    def close_on_escape(self, event=None):
        ...
        # self.worker_get_telemetry.join()
    ...
    def closing(self):
        ...
        """
        while self.worker_get_telemetry.is_alive() or self.worker_database.is_alive():
            time.sleep(0.7)
        return 0
